...
measure denoted by :math:`\mu`, in both cases where where the measure has
independent marginals and dependent marginals.
...
The orthogonality of the functions implies taht:
...
basis orthonormal with respect to the
marginal distribution :math:`\mu_i`. The tensorized multivariate  basis is defined by:
...
- *polynomials*: the associated distribution :math:`\mu_i` can be continuous
  ...
  known (see :ref:`orthogonal_polynomials`).
...
Soize-Ghanel in [soizeghanem2004]_ defined by:
...
Furthermore, this basis generates approximation subspaces of poor quality. Indeed, for most
copulas, the density :math:`c` tends to infinity at :math:`\vect{0}` or :math:`\vect{1}` (and
possibly at other points). As a consequence, all the basis functions
:math:`(\psi_{\vect{\alpha}})_\vect{\alpha}` vanish at these points and take small values in
their neighborhoods. Therefore, the coefficients in the expansion of a function that attains
large values at these points must necessarily be of large magnitude. This leads to numerical
stability issues, both for the computation of the expansion coefficients and for the evaluation
of the metamodel.

That is the reason why the use of such an multivariate  orthonormal basis is not recommended.
