...
Least squares polynomial meta model
-----------------------------------

Instead of replacing the model  :math:`\model(\vect{x})` with a
...
simplicity, a *scalar* model  :math:`y=\model(\vect{x})` will
be considered from now on. \sampleSizeonetheless, the following derivations hold
for a vector-valued.

In the following, one considers global approximations of the model using:

- a linear function, i.e. a polynomial of degree one;

  .. math::

      y  \approx  \widehat{\model}(\vect{x})  =  a_0 \, + \,  \sum_{i=1}^{\inputDim} a_{i} x_i

  where :math:`(a_j  \, , \, j=0,\dots,\inputDim)` is a set of unknown
  coefficients.
...
- a quadratic function, i.e. a polynomial of degree two.
  ...
  .. math::
         ...
         y  \approx  \widehat{\model}(\vect{x})  =  a_0 \, + \,  \sum_{i=1}^{\inputDim} a_{i} x_i \, + \,
         \sum_{i=1}^{\inputDim} \sum_{j=1}^{\inputDim} a_{i,j} x_i x_j
...
:math:`\inputDim + 1` (resp. to :math:`1 + 2\inputDim + \inputDim (\inputDim - 1)/2`) when
...
A *global* approximation of the model over its whole
...
.. math::
    \cJ(\vect{a}) = \sum_{i=1}^\sampleSize \left( y^{(i)} - \Tr{\vect{a}} \vect{\psi}\left(\vect{x}^{(i)}\right) \right)^2.

Let :math:`\vect{y} = \Tr{(y^{(1)},\dots,y^{(\sampleSize)})} \in \Rset^{\sampleSize}` be the
...
for :math:`i = 1, \dots, \sampleSize` and :math:`j = 0, \dots, P - 1`.
A necessary condition for having a solution is that the size :math:`\sampleSize`
