...
- If :math:`\mu_{\inputRV}` has dependent components, we can use an
  ...
  :math:`\mu_{\inputRV}` into a measure :math:`\mu_{\vecxt{U}}` with independent marginals. We can
  also use the Soize-Ghanem basis but this usage is not recommended.
...
least-squares problem :eq:`metaModeleF`. Indeed, if the basis
:math:`\left(\psi_k\right)_{k \in I_n}` is
orthonormal with respect to :math:`\mu_{\inputRV}`, then the design matrix of
the least squares problem is well-conditioned. In the other case, the design matrix might be bad-conditioned, 
...
for :math:`k\in I_n`. This integral can computed unsing  a quadrature rule (see the documentation
