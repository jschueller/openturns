...
# %%
# We perform exact conditional sampling of :math:`\vect{Y}` from a product of univariate truncated normal distributions.
marginals_trunc = [
...
# %%
# Extract nominal values of the parameters.
...
# .. math::
#
#     \vect{\theta}|\vect{Y},\tau \sim \mathcal N_4( \vect{\mu}_n ; \mat{\Sigma}_n ){\bf 1}_{\{\vect{\theta}_{\min} \leq \theta \leq \vect{\theta}_{\max}\}},
#
# with
#
# .. math::
...
#     \mat{\Sigma}_n = \tau^{-1}(\mat{\Sigma}_0^{-1} + X^\top (\mat{I}_n + \tau \mat{Q}^{-1})^{-1} X)^{-1}.
#
...
# .. math::
#
#     \vect{\mu}_n =  (\mat{X}^\top (\mat{I}_n + \tau \mat{Q}^{-1})^{-1} \mat{X})^{-1}(X^\top (\mat{I}_n + \tau \mat{Q}^{-1})^{-1} \vect{Y})\\
...
# %%
# Step 1 : Create associated :class:`~openturns.PythonRandomVector`
...
#     \tau\in[\tau_{\min};\tau_{\max}]} \mathcal N(\vect{Y}^{obs}|\vect{\theta} X; \tau^{-1} \mat{I}_n + \mat{Q}^{-1}).
#
