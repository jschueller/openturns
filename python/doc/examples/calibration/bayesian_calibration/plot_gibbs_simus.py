...
#   :math:`\varepsilon \sim \mathcal N(0, \tau^{-1}I_{n}+\mat{Q}^{-1})`,
#
...
#
#    \vect{Y} | \vect{\theta}, \tau
#    {\sim} \mathcal N_{n}(\mat{X} \vect{\theta} ; \tau^{-1} \mat{I_n} +\mat{Q}^{-1}),
#
...
#    (2\pi)^{-\frac{n}{2}}
#    {\rm det}(\tau^{-1} \mat{I_n}+\mat{Q}^{-1})^{-\frac{1}{2}}
#    e^{-\frac{1}{2}
#    ||Y - \mat{X} \vect{\theta} ||_{\tau^{-1} \mat{I_n} +\mat{Q}^{-1}}^2
#    }
#
...
# ~~~~~~~~~~~~
#
# -  The presence of a composite matrix :math:`\tau^{-1} \mat{I_n}+i\mat{Q}^{-1}` makes
#    estimation more complex than with a spherical one
#    :math:`\tau^{-1}\mat{I_n},` since we would then have explicit (closed-form)
...
#    variables :math:`X_i` are derived from the epicentral distance to the
...
X = np.array(ot.ComposedDistribution([ot.Dirac(1.0), 10.0 * ot.Normal()]).getSample(n))
...
theta_true = np.array([0.0, 1.0]).reshape(-1,1)
p = len(theta_true)
tau_true = 0.1
...
cloud = ot.Cloud(X[:,1], Yobs_sim)
cloud.setPointStyle('bullet')
graph.add(cloud)
curve = ot.Curve(X[:,1], mean_true)
...
# .. math::
# 
#    \vect{\theta} | \tau \sim \mathcal N_{p}(\mu_0; \tau^{-1}\Sigma_0) \\
...
# .. math::
# 
#    \mu_0, \Sigma_0^{-1}, a_0, b_0 \equiv 0
# 
...
# imposing bounds :math:`\theta_{\min}, \theta_{\max}, \tau_{\min}, \tau_{\max}`
# on all parameters following:
# 
# .. math::
# 
#    \vect{\theta} | \tau \sim \mathcal N_{p}(\mu_0; \tau^{-1}\Sigma_0) {\bf 1}_{\{\vect{\theta}_{\min} \leq \vect{\theta} \leq \theta_{\max}\}} ;\\
...
# distribution of all uncertain parameters, including the vector
...
#        & \propto
#        {\bf 1}_{\{\theta_{\min} \leq \vect{\theta} \leq \theta_{\max}\}}\times
#        {\bf 1}_{\{\tau_{\min} \leq \tau \leq \tau_{\max}\}}\times
#        \mathcal N_n (\vect{Y}|\mat{X} \vect{\theta};\tau^{-1} I_{n} + \mat{Q}^{-1}) \times
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# 
# :math:`\pi(\vect{Y}|\vect{\theta},\tau,\vect{Y}^{obs})\propto\prod_{i}\mathcal N_n(Y_{i}|X_{i} \vect{\theta}, \tau^{-1}+q_{i}^{-1}) {\bf 1}_{\left[Y_{i}^{obs}-\frac{\delta}{2}; Y_{i}^{obs}+\frac{\delta}{2}\right[}(Y_{i})`,
# hence the latent variables :math:`Y_i` are updated by simply simulating
# independent univariate truncated normals.
# 

# %%
# Exact conditional sampling of Y from a product of univariate truncated normal distributions
# Step 1: Create associated RandomVector
marginals_trunc = [ot.TruncatedNormal(Yobs_sim[i], 1.0, lower[i], upper[i]) for i in range(len(X))]

trunc_cond_Y = ot.ComposedDistribution(  marginals_trunc  )
RV_Y = ot.RandomVector( trunc_cond_Y )

# Step 2: Source link function (SLF), indicating the parameters of the truncated normal
# as a function of theta and tau.
# TRICK: use the getParameterDescription() method to know what the SLF needs to output
print( trunc_cond_Y.getParameterDescription() )


# %%
# Now, use the getParameter() method to extract nominal values of the parameters as a blueprint for the SLF

gen_params = np.array( trunc_cond_Y.getParameter() )

# NOTE: only the mean and std need to be changed, the upper and lower bounds remain fixed:

def py_link_function_y(x):
    """
    source link function for Y conditional density
...
# .. math::
# 
#     \vect{\theta}|\vect{Y},\tau \sim \mathcal N_4( \mu_n ; \Sigma_n ){\bf 1}_{\{\vect{\theta}_{\min} \leq \theta \leq \vect{\theta}_{\max}\}},
# 
# with
# 
# .. math::
# 
#     \mu_n = \mu_0 + \Sigma_0X^\top(\mat{X} \Sigma_0 \mat{X}^\top + \mat{I_n} + \tau \mat{Q}^{-1})^{-1}(\vect{Y} - \mat{X} \mu_0) \\
#     \Sigma_n = \tau^{-1}\Sigma_0 - \tau^{-1}\Sigma_0 \mat{X}^\top (\mat{X} \Sigma_0 \mat{X}^\top + \mat{I_n} + \tau \mat{Q}^{-1})^{-1} \mat{X} \Sigma_0,
# 
# or equivalently, thanks to the matrix Woodsbury identity:
# 
# .. math::
# 
#     \mu_n =  (\Sigma_0^{-1} + \mat{X}^\top (\mat{I_n} + \tau \mat{Q}^{-1})^{-1} \mat{X})^{-1}(\Sigma_0^{-1} \mu_0 + \mat{X}^\top (\mat{I_n} + \tau \mat{Q}^{-1})^{-1} \vect{Y})\\
#     \Sigma_n = \tau^{-1}(\Sigma_0^{-1} + X^\top (\mat{I_n} + \tau \mat{Q}^{-1})^{-1} X)^{-1}.
# 
...
# .. math::
# 
#     \mu_n =  (\mat{X}^\top (\mat{I_n} + \tau \mat{Q}^{-1})^{-1} \mat{X})^{-1}(X^\top (\mat{I_n} + \tau \mat{Q}^{-1})^{-1} \vect{Y})\\
#     \Sigma_n = \tau^{-1}(\mat{X}^\top (\mat{I_n} + \tau \mat{Q}^{-1})^{-1} \mat{X})^{-1}.
# 
...
# %%
# Exact simulation from theta's conditional posterior distribution using box-constrained multivariate normal


# %%
# Step 1 : Create associated Random Vector
...
class BoxConstrainedNormal(ot.PythonDistribution):
    ...
    """
    def __init__(self, d=2, mu=np.zeros(2), Sigma=np.eye(2), r=np.zeros(2), s=np.ones(2)):
    ...
    def getParameter(self):
        return np.concatenate( [self.mu.ravel(), self.Sigma.ravel(), self.r.ravel(), self.s.ravel()] )
...
# %%
# Step 2 : Link function, giving the parameters of the box-constrained normal in function of tau and Y values
...
def py_link_function_theta(x):
    ...
    return np.concatenate( [mu_n.ravel(), Sigma_n.ravel(), support.getLowerBound()[:p], support.getUpperBound()[:p]] )
...
#    \propto
#    {\bf 1}_{\{\tau_{\min} \leq \tau \leq \tau_{\max}\}}\times
#    \mathcal N_n(\vect{Y}|\mat{X} \vect{\theta};\tau^{-1} \mat{I_n} + \mat{Q}^{-1}).
# 
...
# %%
# Step 1 : compite tau's conditional posterior density, up to a multiplicative factor
...
#     = \arg\max_{\theta\in[\theta_{\min};\theta_{\max}]
#     \tau\in[\tau_{\min};\tau_{\max}]} \mathcal N(\vect{Y}^{obs}|\vect{\theta} X; \tau^{-1} I + \mat{Q}^{-1}).
# 
...
# posterior mode / mean is given by:
# 
# .. math::
# 
#     \arg\max_{\vect{\theta}} \pi(\vect{\theta}|\vect{Y}^{obs},\vect{Y}=\vect{Y}^{obs},\tau) = (X^\top (\mat{I_n} + \tau \mat{Q}^{-1})^{-1} X)^{-1}(X^\top (\mat{I_n} + \tau \mat{Q}^{-1})^{-1} \vect{Y}):= \mu_n(\tau).
# 
...
# .. math::
# 
#     \widehat\tau = \arg\max_{\tau\in[\tau_{\min};\tau_{\max}]} \mathcal N(\vect{Y}^{obs}|\mu_n(\tau) X; \tau^{-1} I + \mat{Q}^{-1}).
# 
# The optimal value of :math:`\vect{\theta}` is then given by:
# :math:`\widehat\theta = \mu_n(\widehat\tau).`
...
def mu_n(tau):
    ...
    x[p] = tau 
    # trick: posterior conditional mean is computed by the source link function
...
# %%
# Final step : create MetropolisHastings objects for each block 
...
rvmh_theta = ot.RandomVectorMetropolisHastings(RV_theta, initialState, mi_theta, link_function_theta)

log_pdf_tau = ot.PythonFunction(len(x), 1, py_log_density)
rwmh_tau = ot.RandomWalkMetropolisHastings(log_pdf_tau, support, initialState,  proposal, [p])
