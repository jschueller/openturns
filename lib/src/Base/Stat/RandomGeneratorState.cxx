...
void RandomGeneratorState::load(Advocate & adv)
{
  ...
  // The "index_" attribute was written by the former DSFMT-based generator.
  // Its presence signals a state that cannot be restored by the current
  // PCG64 generator (the internal state representations are incompatible),
  // so we refuse to load it rather than silently producing a different stream.
  if (adv.hasAttribute("index_"))
    throw InvalidArgumentException(HERE) << "RandomGeneratorState was saved with the obsolete DSFMT generator, "
      << "which is no longer supported; the saved state cannot be restored. "
      << "Re-seed the generator with RandomGenerator.SetSeed() instead.";
